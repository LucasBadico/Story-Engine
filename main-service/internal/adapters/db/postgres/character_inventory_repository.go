...
func (r *CharacterInventoryRepository) Create(ctx context.Context, inventory *rpg.CharacterInventory) error {
	query := `
		INSERT INTO character_inventory (id, character_id, item_id, quantity, slot_id, is_equipped, custom_name, custom_stats, acquired_at)
		VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
	`
	_, err := r.db.Exec(ctx, query,
		inventory.ID, inventory.CharacterID, inventory.ItemID, inventory.Quantity,
...
func (r *CharacterInventoryRepository) GetByID(ctx context.Context, id uuid.UUID) (*rpg.CharacterInventory, error) {
	query := `
		SELECT id, character_id, item_id, quantity, slot_id, is_equipped, custom_name, custom_stats, acquired_at
	...
	err := r.db.QueryRow(ctx, query, id).Scan(
		&ci.ID, &ci.CharacterID, &ci.ItemID, &ci.Quantity, &slotID,
...
func (r *CharacterInventoryRepository) GetByCharacterAndItem(ctx context.Context, characterID, itemID uuid.UUID, slotID *uuid.UUID) (*rpg.CharacterInventory, error) {
	var query string
	var args []interface{}
	...
	if slotID != nil {
		query = `
			SELECT id, character_id, item_id, quantity, slot_id, is_equipped, custom_name, custom_stats, acquired_at
			FROM character_inventory
			WHERE character_id = $1 AND item_id = $2 AND slot_id = $3
		`
		args = []interface{}{characterID, itemID, slotID}
	} else {
		query = `
			SELECT id, character_id, item_id, quantity, slot_id, is_equipped, custom_name, custom_stats, acquired_at
			FROM character_inventory
			WHERE character_id = $1 AND item_id = $2 AND slot_id IS NULL
		`
		args = []interface{}{characterID, itemID}
	}
	...
	err := r.db.QueryRow(ctx, query, args...).Scan(
		&ci.ID, &ci.CharacterID, &ci.ItemID, &ci.Quantity, &slotIDNull,
		&ci.IsEquipped, &customName, &customStats, &ci.AcquiredAt)
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return nil, &platformerrors.NotFoundError{
...
func (r *CharacterInventoryRepository) ListByCharacter(ctx context.Context, characterID uuid.UUID) ([]*rpg.CharacterInventory, error) {
	query := `
		SELECT id, character_id, item_id, quantity, slot_id, is_equipped, custom_name, custom_stats, acquired_at
		FROM character_inventory
...
func (r *CharacterInventoryRepository) ListEquippedByCharacter(ctx context.Context, characterID uuid.UUID) ([]*rpg.CharacterInventory, error) {
	query := `
		SELECT id, character_id, item_id, quantity, slot_id, is_equipped, custom_name, custom_stats, acquired_at
		FROM character_inventory
...
func (r *CharacterInventoryRepository) Update(ctx context.Context, inventory *rpg.CharacterInventory) error {
	...
	_, err := r.db.Exec(ctx, query,
		...
		inventory.CustomName, inventory.CustomStats)
	return err
}

// Delete deletes a character inventory entry
func (r *CharacterInventoryRepository) Delete(ctx context.Context, id uuid.UUID) error {
	query := `DELETE FROM character_inventory WHERE id = $1`
	_, err := r.db.Exec(ctx, query, id)
	return err
}

// DeleteByCharacter deletes all inventory entries for a character
func (r *CharacterInventoryRepository) DeleteByCharacter(ctx context.Context, characterID uuid.UUID) error {
	query := `DELETE FROM character_inventory WHERE character_id = $1`
